...
const ActivityCancelledTab = () => {
    ...
    useEffect(() => {
        if (user?.id) {
            handleGetPayments(
                `current=1&pageSize=10&booking__user_id=${user.id}&booking__service_type=${ServiceType.ACTIVITY}&status=${PAYMENT_STATUS.CANCELLED}&${sortVal}&booking__booking_code=${bookingCode}`
            );
        }
