...
import {
  callBook,
  callFetchAmenities,
  callFetchPhysicalRoom,
  callFetchRoomQuery,
} from "../../config/api";
import PhysicalRoomSelectorModal from "./PhysicalRoomSelectorModal";
...
const RoomOptionsSection = ({
  ...
  const [showPhysicalRoomSelector, setShowPhysicalRoomSelector] =
    useState(false);
  const [selectedPhysicalRooms, setSelectedPhysicalRooms] = useState([]);
    ...
    try {
      const updatedPayload = {
        ...
        room_details: {
          ...
          physical_room_codes: physicalRoomCodes,
        },
      };
  ...
  const confirmBookingWithTime = async () => {
    ...
    const checkOutDateTime = `${startDate}T${checkOutTime.format("HH:mm")}:00`;
    ...
    setShowPhysicalRoomSelector(true);
  };
  ...
  const confirmPhysicalRoomSelection = async (selectedRooms) => {
    setSelectedPhysicalRooms(selectedRooms);
    setShowPhysicalRoomSelector(false);
    // Sau khi chọn phòng, tiến hành booking
    await proceedWithBooking(
      selectedRoomForBooking.id,
      ...
      endDate,
      selectedRoomForBooking,
      selectedRooms.map((r) => r.code) // physical_room_codes
    );
  };
  ...
  return (
    <>
        ...
        {rooms.map((room) => {
          ...
          if (room.stay_type === "dayuse") {
            ...
            if (!isNaN(originalPrice)) {
              displayPrice = originalPrice.toLocaleString("vi-VN");
          ...
          if (
            ...
            discountedPrice =
              originalPrice * (1 - room.promotion.discount_percent / 100);
          }
            ...
            >
                ...
                <div className="mb-3 p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded">
                  ...
                  <div className="text-sm text-yellow-800 mt-1">
                    {room.promotion.discount_percent
                      ...
                      : ""}
                    <span className="ml-2">
                      (Từ {room.promotion.start_date?.slice(0, 10)} đến{" "}
              ...
              <div className="grid grid-cols-[30%_40%_30%] gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600">{priceUnit}</p>
                  <button
      ...
      <PhysicalRoomSelectorModal
        isOpen={showPhysicalRoomSelector}
        onClose={() => setShowPhysicalRoomSelector(false)}
        onConfirm={confirmPhysicalRoomSelection}
      ...
      <div className="bg-white p-6 rounded-lg max-w-md w-full">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            onChange={(e) => setSelectedTime(dayjs(e.target.value, "HH:mm"))}
