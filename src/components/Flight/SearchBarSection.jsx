...
const SearchBarSection = ({ 
  ...
  // Nếu parent truyền origin/destination (ví dụ khi chọn chiều về), ưu tiên dùng props
  ...
  // Nếu có propOrigin/propDestination thì luôn ưu tiên hiển thị theo props (dùng cho chiều về round-trip)
  ...
  // Remove explicit tripType, infer from returnDate
  ...
  const handleSearch = () => {
    // Determine tripType based on returnDate
  ...
  // Keyboard navigation cho dropdown hạng ghế
  useEffect(() => {
  ...
  return (
    <div className="flex items-center gap-2 bg-white p-2 rounded-full shadow-md overflow-x-auto whitespace-nowrap z-0">
      {/* Loại chuyến bay: đã bỏ, tự động xác định theo ngày về */}

      {/* Điểm đi, nút swap, điểm đến */}
