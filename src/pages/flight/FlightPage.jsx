...
const FlightPage = () => {
  ...
  const handleSelectFlight = useCallback(async (flight, seatClassSelected) => {
    if (tripType === 'one-way') {
      ...
      const bookingBody = {
        ...
        flight_detail: [{
          flight: flight.id,
          seat_class: seatClassSelected ? seatClassSelected.seat_class : seatClass,
      ...
      try {
        ...
        if (bookingId) {
          navigate(`/book?booking_id=${bookingId}&type=${ServiceType.FLIGHT}`);
        }
      } catch (err) {
          ...
          flight_detail: [
            {
              ...
              seat_class: selectedOutbound.seatClassSelected ? selectedOutbound.seatClassSelected.seat_class : seatClass,
              num_passengers: passengers
            },
            {
              flight: flight.id,
  ...
  if (tripType === 'round-trip' && step === 'inbound') {
    ...
    listTitle = `Chọn chuyến về: ${getAirportName(destinationParam)} → ${getAirportName(originParam)}`;
  } else {
    listTitle = `Chọn chuyến đi: ${getAirportName(originParam)} → ${getAirportName(destinationParam)}`;
  }
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0 space-y-4">
              ...
              <p className="text-sm text-gray-600">
                ...
                {tripType === 'round-trip' ? `Khứ hồi - ${passengers} hành khách` : `Một chiều - ${passengers} hành khách`}
                {tripType === 'round-trip' && step === 'inbound' && selectedOutbound && (
                  <span className="block mt-2 text-green-700 text-base">Đã chọn chiều đi: {getAirportName(originParam)} → {getAirportName(destinationParam)} ({selectedOutbound.flight.airline?.name}, {selectedOutbound.flight.departure_time?.slice(0,16).replace('T',' ')})</span>
                )}
              </p>
              <FlightSortFilter />
              <FlightList 
