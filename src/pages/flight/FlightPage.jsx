...
const FlightPage = () => {
  ...
  // Handler khi chọn flight và ghế
  const handleSelectFlight = useCallback(
    async (flight, seatClassSelected, selectedSeats) => {
      if (tripType === "one-way") {
        ...
        // round-trip
        if (step === "outbound") {
          setSelectedOutbound({ flight, seatClassSelected, selectedSeats });
          setStep("inbound");
        } else if (step === "inbound") {
          setSelectedInbound({ flight, seatClassSelected, selectedSeats });
            ...
            flight_detail: [
              {
                ...
                seat_class: selectedOutbound.seatClassSelected
                  ? selectedOutbound.seatClassSelected.seat_class
                  : seatClass,
                num_passengers: passengers,
                seat_numbers: selectedOutbound.selectedSeats || [],
              },
              {
                flight: flight.id,
                seat_class: seatClassSelected
                ...
                {tripType === "round-trip" &&
                  ...
                  selectedOutbound && (
                    <span className="block mt-2 text-green-700 text-base">
                      Đã chọn chiều đi: {getAirportName(originParam)} →{" "}
                      {getAirportName(destinationParam)} (
                      {selectedOutbound.flight.airline?.name},{" "}
